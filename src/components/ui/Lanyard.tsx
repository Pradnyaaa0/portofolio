...
import { createCardCompositeTexture, createStrapCanvasTexture } from './lanyard-texture';
...
function Band({
  ...
  const cardMap = useMemo(() => {
    return createCardCompositeTexture({
      baseMap: materials?.base?.map as THREE.Texture | undefined,
      frontImgEl,
      backImgEl,
      options: {
        headerText,
  ...
  }, [materials?.base?.map, frontImgEl, backImgEl, headerText, nameLine1, nameLine2, roleText, githubText, emailText, imageFit]);
  ...
  const strapTexture = useMemo(() => {
    ...
    return createStrapCanvasTexture(nameLine1, texture);
